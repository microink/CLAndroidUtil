...
import com.clandroid.plog.helper.FileHelper;
import com.clandroid.plog.internal.PLogExceptionHandler;
import com.clandroid.plog.internal.PLogLifecycle;
import com.clandroid.plog.internal.PLogReceiver;
import com.clandroid.plog.record.LogRecorder;
import com.clandroid.plog.record.impl.LogRecorderImpl;
import com.clandroid.plog.upload.PrepareUploadListener;
import com.clandroid.plog.upload.UploadListener;
import com.clandroid.plog.util.AppUtil;
import com.clandroid.plog.util.LogUtil;
import com.clandroid.plog.util.NetUtil;
...
enum PLogManagerImpl implements PLogManager {
    ...
    private boolean initLogDir() {
        if (TextUtils.isEmpty(mConfig.getLogDir())) {
            ...
            if (!AppUtil.inMainProcess(mConfig.application)) {
                // 不是主进程，那么创建一个文件夹去放这些log
                String processName = AppUtil.getCurProcessName(mConfig.application);
                logDir = mConfig.getLogDir() + File.separator + processName;
            }
            mConfig.setLogDir(logDir);
